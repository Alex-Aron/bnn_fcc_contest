...
module bnn_fcc #(
  ...
  initial begin
    ...
    for (int i = 0; i < LAYERS - 1; i++) begin
      if (PARALLEL_NEURONS[i] != PARALLEL_INPUTS)
        $fatal(
            1, "bnn_fcc requires PARALLEL_NEURONS to match PARALLEL_INPUTS in all hidden layers"
        );
    end
  ...
  always_comb begin : argmax
    if (PARALLEL_NEURONS[LAYERS-1] != TOPOLOGY[LAYERS])
      $fatal(
          1,
          "bnn_fcc currently requires output layer neurons to match PARALLEL_NEURONS for that layer"
      );
